 ...
 */
package com.theoctacoder.artryon.helper
...
class ARCoreSessionLifecycleHelper(
    ...
    private fun tryCreateSession(): Session? {
        ...
        return try {
            ...
            val session = Session(activity, features)
            println("CAMERAAAAAA ${canSwitchCamera(session)}")
            if(canSwitchCamera(session)) {
                val cameraConfigFilter = CameraConfigFilter(session)
                val desiredFacingDirection =
                    if (isFrontFacing) FacingDirection.FRONT else FacingDirection.BACK
                cameraConfigFilter.setFacingDirection(desiredFacingDirection)


                // Get the list of supported camera configs
                val cameraConfigs = session.getSupportedCameraConfigs(cameraConfigFilter)

                if (!cameraConfigs.isEmpty()) {
                    // Choose the first config that matches our filter
                    session.cameraConfig = cameraConfigs[0]
                } else {
                    // Handle the case where the desired camera is not available
                    Log.e("ARCore", "Desired camera configuration is not available")
                }
            }
            session
        } catch (e: Exception) {
...
//            exceptionCallback?.invoke(e)
    ...
    private fun canSwitchCamera(session: Session): Boolean {
        val frontCameraFilter = CameraConfigFilter(session)
            .setFacingDirection(FacingDirection.FRONT)
        val backCameraFilter = CameraConfigFilter(session)
            .setFacingDirection(FacingDirection.BACK)

        val hasFrontCamera = !session.getSupportedCameraConfigs(frontCameraFilter).isEmpty()
        val hasBackCamera = !session.getSupportedCameraConfigs(backCameraFilter).isEmpty()
